...
As well as frequently obtaining better quality solutions at the end of the optimisation process, DAGA also finds significantly better solutions than the GA at earlier stages in the optimisation process on a number of the tested problems. Table~\ref{table:main_results} shows the mean best solution found after different numbers of function evaluations have passed. From this we see that on all of the problems, apart from RR, DAGA finds significantly better solutions than the GA after a quarter of function evaluations have passed. On the HIFF and RR functions, the GA is able to locate optimal solutions eventually but not consistently. DAGA is statistically significantly better after 25\%, 50\% and 75\% of evaluations have passed on the HIFF. On this difficult problem, DAGA finds the optimum after 230,000 evaluations on average, with 200,000 in the fastest case and 285,000 in the slowest. At its fastest, the GA finds the optimum after 300,000 evaluations (mean 332,000 on the successful attempts), although on half the trials it fails to reach the optimum after 500,000. On RR, DAGA has a better mean score from the first quarter onwards, but the GA regularly reaches the optimum solution and their scores are not significantly different statistically (p=0.13 from 25\% to 50\%).

Table~\ref{} presents the parameter values for the chosen configuration of DAGA and the GA used to solve each problem. A noticeable trend is that two of the problems use a very high corruption level (Knapsack 500 and RR) while the others use a low number. The high corruption level (CL) instances could be seen as easier to solve, as the knapsack instance, although larger than Weing8, has only one set of constraints and RR has few local optima. Figure~\ref{figure:corruption} shows the effect of varying CL from 0.0 - 1.0 on the 500-bit knapsack. It is clear that CL has a large impact on the convergence speed, with high CLs finding high quality solutions very quickly.
...
\begin{figure}[t!]
\center
\includegraphics[width = 0.8\textwidth]{images/corruption_tests.png}

\caption{Showing the fitness of the best solution in the population over time of DAGA with different corruption levels on the Knapsack 500 problem, averaged over 5 trials.} 
\label{figure:corruption}
\end{figure}
